...
func TestPromoteElasticRequiresSnapshotEndpoint(t *testing.T) {
	t.Setenv(contract.EnvElasticRuntime, "1")
	t.Setenv("PATH", t.TempDir())

	store, err := registry.Open(t.TempDir() + "/promote-elastic.sqlite")
	if err != nil {
		t.Fatal(err)
	}
	defer store.Close()
	...
	o := orch.New(store, job.NewSQLiteQueue(store), branch.New(t.TempDir(), store),
		runtime.New(8792, "", "us-east-1", "s3://cellp-celld", "k", "s"), &artifact.Store{LocalDir: cfg.ArtifactsDir}, cfg)
	...
	// No active route / endpoint in snapshot.

	err = o.Promote(ctx, "demo", "v-new")
	if err == nil {
		t.Fatal("expected promote eligibility error")
	}
}

func TestPromoteElasticCommitProdSingleRevisionBump(t *testing.T) {
	t.Setenv(contract.EnvElasticRuntime, "1")
	t.Setenv("PATH", t.TempDir())

	artifactsDir := t.TempDir()
	store, err := registry.Open(t.TempDir() + "/promote-rev.sqlite")
	if err != nil {
		t.Fatal(err)
	}
	defer store.Close()
	...
	revBefore, _ := store.GetRouteRevision(ctx)
	if err := o.Promote(ctx, "demo", "v-new"); err != nil {
		t.Fatal(err)
	}
	revAfter, _ := store.GetRouteRevision(ctx)
	// deactivate old (+1) + CommitProdPromote cutover (+1)
	if revAfter < revBefore+2 {
		t.Fatalf("expected >=2 revision bumps, before=%d after=%d", revBefore, revAfter)
	}
}
