...
| S40 | **Next.js (OpenNext)** | **不支持** | — | **实验 v12 + 官方套件**：固定基础 App Router sample 通过 preview gate 后 promote；OpenNext `1.14.0` 官方核心套件为 119 declarations / 5 skip / 114 runnable，其中 56 实际执行为 49 stable passed、6 stable failed、1 confirmed flaky，另 58 因 no-bundle `*.wasm?module` 未进入 celld manifest而在 activation/module linking 阶段阻塞。不是完整支持率，保持 AD-13 tier-1 不支持 · [NEXT-OPENNEXT-CELLP.md](./plans/NEXT-OPENNEXT-CELLP.md) |
...
| CloudFlare-ImgBed | **已纳入 S31** | **支持** · `dev/examples/support-imgbed/` · Workers `deploy/worker` · KV+R2+`IMAGES`（celld Partial：transform/output/info）· `_routes.json` 静态优先 · prod **v8**（2026-09-03） |
...
| Serverless DNS | **已纳入 S37 · 不支持** | **S37** · [serverless-dns/serverless-dns](https://github.com/serverless-dns/serverless-dns) · overlay `dev/examples/support-serverless-dns/`（webpack → wrangler dry-run · build 需 `wget`+`pre.sh` 拉 `u6-*` json · cellp 侧提高 `WORKER_TIMEOUT`/dnsutil cap）· prod **v3** Host `support-serverless-dns.lvh.me` · prod smoke `GET /` / `/configure` **408**（~120s，blocklist 冷启动未完成）· 设计上浏览器访问应 **302** 到 `https://rethinkdns.com/configure?tstamp=…`（外部配置 UI）· DoH **`/dns-query`** · `docs/evidence/support-S37.log` |
...
| 组件 | 状态 |
|------|------|
| Next.js / OpenNext | **不支持（tier-1）**；S30/S40 固定 artifact 通过各自实验 gate；OpenNext `1.14.0` 官方核心套件 114 runnable 中 56 实际执行（49 stable passed、6 stable failed、1 confirmed flaky），另 58 被 celld no-bundle module publication/activation 阻塞。该结果不是任意版本或完整功能支持承诺 · [NEXT-OPENNEXT-CELLP.md](./plans/NEXT-OPENNEXT-CELLP.md) |
