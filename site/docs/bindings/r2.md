...
`cache-objects.json` uses Wrangler's bulk shape: a JSON array of `{ "key": "...", "file": "..." }`.
Relative `file` paths resolve from the manifest directory. Objects are written under
`r2/<BUCKET_NAME>/<key>` in the version fleet bucket on RustFS.

This is the supported import path for OpenNext cache-population output on cellp. It is **not**
an object browser and does not list or delete existing keys.
